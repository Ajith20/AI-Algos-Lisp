...
; and returns it
(select-init (car open_list) (cdr open_list))
)

(defun select-init (car-open cdr-list)
(cond ((null cdr-list) car-open)
      ((> (length (partial_plan-plan_nodes  car-open)) (length (partial_plan-plan_nodes (car cdr-list)))) (select-init (car cdr-list) (cdr cdr-list)))
      (t (select-init car-open (cdr cdr-list))))))
...
(expanding-plans part_plan open_prec open_list action_nodes_existing)
)

(defun expanding-plans (part_plan open_prec open_list nodes_list_effects)
(cond ((null nodes_list_effects) open_list)
      (t (append (expand-plan part_plan open_prec open_list (car nodes_list_effects)) (expanding-plans part_plan open_prec open_list (cdr nodes_list_effects))))))


(defun expand-plan (part_plan open_prec open_list action_node)
(let* ((new_plan (make-partial_plan :plan_nodes (partial_plan-plan_nodes part_plan) :causal_links (append (partial_plan-causal_links part_plan) (list (let ((new_causal (make-causal_link :plan_node_1 action_node :plan_node_2 (open_precondition-node open_prec) :protected_prop (open_precondition-precondition open_prec)))) new_causal))) :order_constraints (append (partial_plan-order_constraints part_plan) (list (let ((new_order (make-order_constraint :before_node action_node :after_node (open_precondition-node open_prec)))) new_order))) :open_precs (cdr (partial_plan-open_precs part_plan)))))
...
(defun actions_effects (open_prec action_nodes_existing)
(cond ((null action_nodes_existing) ())
      ((effects open_prec (plan_node-effects (car action_nodes_existing))) (append (list (car action_nodes_existing)) (actions_effects open_prec (cdr action_nodes_existing))))
      (t (actions_effects open_prec (cdr action_nodes_existing)))))


(defun effects (open_prec list)
(cond ((null list) ())
      ((equal (precondition-pname open_prec) (car list)) t)
      (t (effects open_prec (cdr list)))))

(defun make_consistent (new_plan)
(cond ((make_consis (inconsistent_causal (partial_plan-causal_links new_plan) (partial_plan-plan_nodes new_plan)) new_plan) (make_consis (inconsistent_causal (partial_plan-causal_links new_plan) (partial_plan-plan_nodes new_plan)) new_plan))
      (t ())))


(defun make_consis (list new_plan)
...
(cond (t new_partial_plan))))


;returns what action_and causal link function returns
(defun inconsistent_causal (causal_links_list actions_list)
(cond ((null actions_list) ())
      (t (append (effects_causal causal_links_list (plan_node-effects (car actions_list)) (car actions_list)) (inconsistent_causal causal_links_list (cdr actions_list))))))

;Sends each effect
(defun effects_causal (causal_links_list action_effects action)
(cond ((null action_effects) ())
      (t (append (action_and_causal causal_links_list (car action_effects) action) (effects_causal causal_links_list (cdr action_effects) action)))))


;Returns causal link and action in a list

(defun action_and_causal (causal_links_list effect action)
(cond ((null causal_links_list) ())
      ...
      (t (action_and_causal (cdr causal_links_list) effect action))))
...
(expanding-plans1 part_plan open_prec open_list action_nodes_new)
)


(defun expanding-plans1 (part_plan open_prec open_list nodes_list_effects)
(cond ((null nodes_list_effects) open_list)
      (t (append (expand-plan1 part_plan open_prec open_list (car nodes_list_effects)) (expanding-plans1 part_plan open_prec open_list (cdr nodes_list_effects))))))


(defun expand-plan1 (part_plan open_prec open_list action_node)
(let* ((new_plan (make-partial_plan :plan_nodes (append (partial_plan-plan_nodes part_plan) (list action_node)) :causal_links (append (partial_plan-causal_links part_plan) (list (let ((new_causal (make-causal_link :plan_node_1 action_node :plan_node_2 (open_precondition-node open_prec) :protected_prop (open_precondition-precondition open_prec)))) new_causal))) :order_constraints (append (partial_plan-order_constraints part_plan) (list (let ((new_order1 (make-order_constraint :before_node (get-start-node (partial_plan-plan_nodes part_plan)) :after_node action_node))) new_order1)) (list (let ((new_order2 (make-order_constraint :before_node action_node :after_node (get-finish-node (partial_plan-plan_nodes part_plan))))) new_order2)) (list (let ((new_order (make-order_constraint :before_node action_node :after_node (open_precondition-node open_prec)))) new_order))) :open_precs (append (cdr (partial_plan-open_precs part_plan)) (make_open_precs (plan_node-preconditions action_node) action_node)))))
(cond ((consistent? new_plan) (list new_plan))
      (t (make_consistent new_plan)))))


(defun start_and_finish (action_nodes)
(cond ((null action_nodes) ())
     ((equal (action-act_name (plan_node-act (car action_nodes))) 'start) (append (car action_nodes) (start_and_finish (cdr action_nodes))))
     ((equal (action-act_name (plan_node-act (car action_nodes))) 'finish) (append (car action_nodes) (start_and_finish (cdr action_nodes))))
     (t (start_and_finish (cdr action_nodes)))))
...
(cond ((null new_part_plan) ())
       ((consistent new_part_plan (car (reverse (partial_plan-causal_links new_part_plan))) (partial_plan-plan_nodes new_part_plan)) new_part_plan)
      (t (make_consistent new_part_plan))))



(defun consistent (new_plan causal_link_prec action_nodes)
(cond ((and (consistent_prec new_plan (causal_link-protected_prop causal_link_prec) action_nodes (car (reverse (partial_plan-causal_links new_plan)))) (consistent_effects new_plan (plan_node-effects (causal_link-plan_node_1 causal_link_prec)) (partial_plan-causal_links new_plan) (causal_link-plan_node_1 causal_link_prec))) t)
      (t ()))))

(defun consistent_prec (new_plan prec action_nodes causal_link1)
(cond ((null action_nodes) t)
      ((not (consis_prec prec (plan_node-effects (car action_nodes)) (car action_nodes) causal_link1 new_plan)) ())
      (t (consistent_prec new_plan prec (cdr action_nodes) causal_link1))))) 

(defun consis_prec (prec effects_list car-action_nodes causal_link1 new_plan)
(cond ((null effects_list) t)
      ((null prec) t)
      ((and (equal (literal-pname prec) (literal-pname (car effects_list))) (not (equal (literal-sign prec) (literal-sign (car effects_list)))) (not (or (check_order (causal_link-plan_node_1 causal_link1) car-action_nodes (partial_plan-order_constraints new_plan)) (check_order (causal_link-plan_node_2 causal_link1) car-action_nodes (partial_plan-order_constraints new_plan))))) ())
      (t (consis_prec prec (cdr effects_list) car-action_nodes causal_link1 new_plan))))

(defun make_open_precs (literal_list action_node)
(cond ((null literal_list) ())
      (t (append (list (let ((new_open_prec (make-open_precondition :precondition (car literal_list) :node action_node))) new_open_prec)) (make_open_precs (cdr literal_list) action_node)))))

(defun check_order (node1 node2 order_list)
(cond ((null order_list) ())
      ...
      (t (check_order node1 node2 (cdr order_list)))))

(defun consistent_effects (new_plan effects_list causal_links action_node)
(cond ((null effects_list) t)
      ((not (consis_effects (car effects_list) causal_links action_node (partial_plan-order_constraints new_plan))) ())
      (t (consistent_effects new_plan (cdr effects_list) causal_links action_node))))


(defun consis_effects (effect causal_links action_node order_list)
(cond ((null (cdr causal_links)) t)
      ((and (equal (literal-pname effect) (literal-pname (causal_link-protected_prop (car causal_links)))) (not (equal (literal-sign effect) (literal-sign (causal_link-protected_prop (car causal_links))))) (not (or (check_order (causal_link-plan_node_1 (car causal_links)) action_node order_list) (check_order (causal_link-plan_node_2 (car causal_links)) action_node order_list)))) ())
